...
import { deletePost, getPostByIdCached, getPendientes, updatePost } from "../services/postService";
...
const AdminPanel = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoadingData(true);
      try {
const [reportSnapshot, userSnapshot, pendientesData] = await Promise.all([
          getReports(),
          getUsers(),
          getPendientes(),
        ]);
  ...
  const processAction = async () => {
    ...
    try {
      ...
      if (action.kind === "approve-post") {
        await updatePost(action.post.id, { estado: "aprobado" });
        setPendientes((current) => current.filter((post) => post.id !== action.post.id));
      }

      if (action.kind === "delete-pending-post") {
        await deletePost(action.post.id);
        setPendientes((current) => current.filter((post) => post.id !== action.post.id));
      }
  ...
  const actionCopy = action?.kind === "discard-report"
    ...
    : action?.kind === "delete-content"
      ...
      : action?.kind === "approve-post"
      ? {
          title: "¿Aprobar esta publicación?",
          message: "La publicación volverá a ser visible para todos.",
          label: "Aprobar",
          danger: false,
        }
      : action?.kind === "delete-pending-post"
        ? {
            title: "¿Eliminar esta publicación?",
            message: "Se eliminará de forma irreversible.",
            label: "Eliminar",
            danger: true,
          }
      : {
      ...
      <main className="admin-panel">
        ...
        <div className="admin-tabs" role="tablist" aria-label="Secciones administrativas">
          ...
          <button
            type="button"
            role="tab"
            aria-selected={activeTab === "moderacion"}
            className={activeTab === "moderacion" ? "active" : ""}
        ...
        {loadingData ? (
          ...
          <section className="admin-list" aria-label="Publicaciones en revisión">
            {pendientes.length === 0 ? (
              <p className="admin-empty">No hay publicaciones pendientes de revisión 🎉</p>
            ) : pendientes.map((post) => (
              <article className="admin-card" key={post.id}>
                <div className="admin-card-content">
                  <h2>Post marcado por el filtro</h2>
                  <p>{post.description?.trim() || "Publicación sin texto"}</p>
                  <dl className="admin-meta">
                    <div><dt>Autor</dt><dd><ResolvedUser uid={post.authorId} /></dd></div>
                    <div><dt>Fecha</dt><dd>{formatearFecha(post.createdAt)}</dd></div>
                  </dl>
                </div>
                <div className="admin-card-actions">
                  <button
                    type="button"
                    onClick={() => setAction({ kind: "approve-post", post })}
                  >
                    Aprobar
                  </button>
                  <button
                    type="button"
                    className="admin-danger-button"
                    onClick={() => setAction({ kind: "delete-pending-post", post })}
                  >
                    Eliminar
                  </button>
                </div>
